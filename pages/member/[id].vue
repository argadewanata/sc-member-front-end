    ...
    <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 relative px-4">
        ...
        <div v-else-if="hasPermission" class="bg-white p-4 sm:p-8 md:p-12 lg:p-16 rounded-lg shadow-lg w-full max-w-lg">
            ...
            <div v-if="member">
                ...
                <p><strong>Name:</strong> {{ member.nama_lengkap }}</p>
                <p><strong>Email:</strong> {{ member.email }}</p>
                <p><strong>WhatsApp:</strong> {{ member.nomor_whatsapp }}</p>
                <p><strong>Province:</strong> {{ member.provinsi }}</p>
                <p><strong>City:</strong> {{ member.kota }}</p>
                <p><strong>District:</strong> {{ member.kecamatan }}</p>
                <p><strong>Sub-district:</strong> {{ member.kelurahan }}</p>
                <p><strong>Full Address:</strong> {{ member.alamat_lengkap }}</p>
                <p><strong>Gender:</strong> {{ member.jenis_kelamin }}</p>
                <p><strong>Date of Birth:</strong> {{ member.tanggal_lahir }}</p>
                <p><strong>Last Education:</strong> {{ member.pendidikan_terakhir }}</p>
                <p><strong>Profession:</strong> {{ member.profesi }}</p>
                <p><strong>Reference:</strong> {{ member.referensi }}</p>
                <p><strong>Expectation:</strong> {{ member.harapan }}</p>
